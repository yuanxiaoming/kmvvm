...
class PrefsGenerator(
    ...
    companion object {
        ...
        private val CLASS_NAME_CONTEXT_MANAGER =
            ClassName("com.framework.utils.manager", "ContextManager")
    ...
    override fun process(resolver: Resolver) {
        ...
        val list = resolver.getKSClassDeclarations<com.framework.annotation.Prefs>()
        if (list.isNotEmpty()) {
            generate(list)
        }
    }
    ...
    private fun generate(list: List<KSClassDeclaration>) {
        list.forEach {
            val prefs = it.getAnnotationsByType(com.framework.annotation.Prefs::class).first()
    ...
    private fun addFuns(ksClassDeclaration: KSClassDeclaration): MutableList<FunSpec> {
        ...
        fieldElements.forEach {
            val annotations =
                it.getAnnotationsByType(com.framework.annotation.PrefsField::class).toList()
     ...
     */
    private fun addCodeBlock(prefs: com.framework.annotation.Prefs, className: String): CodeBlock {
